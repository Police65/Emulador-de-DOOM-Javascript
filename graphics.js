...
graphics.drawPixel = function (color, x, y) {
  if (x >= 0 && x < this.width && y >= 0 && y < this.height) {
    if (this.useBuffer) {
      ...
      this.ctx.fillStyle =
        "rgb(" +
        playpal.getPalette(color)[0] +
        ", " +
        playpal.getPalette(color)[1] +
        ", " +
        playpal.getPalette(color)[2] +
        ")";
  ...
  if (this.useBuffer) {
    for (var x = 0; x < this.width; x++) {
      for (var y = 0; y < this.height; y++) {
        ...
        if (
          playpal.current != playpal.old ||
          this.screenBuffer[x][y] != this.oldBuffer[x][y]
        ) {
          this.ctxData.data[c] = playpal.getPalette(this.screenBuffer[x][y])[0];
          this.ctxData.data[c + 1] = playpal.getPalette(
            this.screenBuffer[x][y]
          )[1];
          this.ctxData.data[c + 2] = playpal.getPalette(
            this.screenBuffer[x][y]
          )[2];
